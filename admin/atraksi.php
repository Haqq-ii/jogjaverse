...
$colNama   = pick_col($colNames, ["nama_atraksi", "nama", "judul", "nama_tempat"]);
...
if (isset($_GET['hapus'])) {
  ...
  header("Location: " . BASE_URL . "/admin/atraksi.php");
  exit();
}
...
if (isset($_FILES['gambar_sampul_url']) && is_uploaded_file($_FILES['gambar_sampul_url']['tmp_name'])) {
  ...
  $safeName = uniqid('atr_', true) . "." . strtolower($ext);
...
$kategoriOptions = fetch_options_by_table(
  $koneksi,
  ["kategori", "kategori_atraksi", "kategori_destinasi"],
...
<div class="card">
  <div>
    <h1 class="page-title">Kelola Atraksi</h1>
    <p class="page-sub">Tambah / edit atraksi wisata Jogja</p>
  </div>
  ...
  <div class="toolbar">
    <div class="left">
      <button class="btn" onclick="openModal()">+ Tambah Atraksi</button>
    </div>
    <form method="GET" style="display:flex;gap:10px;align-items:center">
      ...
      <a class="btn secondary" href="<?= BASE_URL ?>/admin/atraksi.php">Reset</a>
    </form>
  </div>
    ...
    <table class="table2">
      ...
      <?php while($row = $data->fetch_assoc()): ?>
        <tr>
          ...
          <td><?= h($row['hari_buka'] ?? '-') ?></td>
          <td><small><?= h(($row['jam_mulai'] ?? '') . ($row['jam_mulai'] ? " - " : "") . ($row['jam_selesai'] ?? '')) ?></small></td>
          ...
          <td class="actions">
            <a class="btn-sm gray" href="<?= BASE_URL ?>/admin/atraksi.php?edit=<?= h($row[$pk]) ?>">Edit</a>
            <a class="btn-sm danger" onclick="return confirm('Hapus atraksi ini?')" href="<?= BASE_URL ?>/admin/atraksi.php?hapus=<?= h($row[$pk]) ?>">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
      </tbody>
...
<div class="modal" id="modal">
  <div class="modal-card">
    <div class="modal-head">
      <h3><?= $edit ? "Edit Atraksi" : "Tambah Atraksi" ?></h3>
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="form-grid">
        ...
        <div>
          <label>hari_buka</label>
          <input type="text" name="hari_buka" value="<?= h($edit['hari_buka'] ?? '') ?>">
        </div>
        <div>
          <label>jam_mulai</label>
          <input type="time" name="jam_mulai" value="<?= h($edit['jam_mulai'] ?? '') ?>">
        </div>
        <div>
          <label>jam_selesai</label>
          <input type="time" name="jam_selesai" value="<?= h($edit['jam_selesai'] ?? '') ?>">
        </div>
        <div class="full">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px;margin-top:10px;">
              <?php foreach ($detailImages as $img): ?>
                <div style="border:1px solid #eee;padding:6px;border-radius:10px;text-align:center;">
                  ...
                  <a class="btn-sm danger" style="margin-top:6px;display:inline-block;" onclick="return confirm('Hapus gambar ini?')" href="<?= BASE_URL ?>/admin/delete_detail_gambar.php?id_galeri=<?= h($img['id_galeri']) ?>&redirect=atraksi&ref_id=<?= h($edit[$pk]) ?>">Hapus</a>
                </div>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>
