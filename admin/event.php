...
$colNama    = pick_col($colNames, ["judul", "nama_event", "nama", "title"]);
$colLok     = pick_col($colNames, ["lokasi", "tempat", "venue", "alamat", "kota"]);
$colMulai   = pick_col($colNames, ["mulai_pada", "tanggal_mulai", "tgl_mulai", "mulai", "start_date", "start"]);
$colSelesai = pick_col($colNames, ["selesai_pada", "tanggal_selesai", "tgl_selesai", "selesai", "end_date", "end"]);
$colStatus  = pick_col($colNames, ["status", "status_event"]);
$colFoto    = pick_col($colNames, ["gambar_sampul_url", "banner", "poster_url", "foto_url", "gambar_url", "cover"]);
...
if (isset($_GET['hapus'])) {
  ...
  header("Location: " . BASE_URL . "/admin/event.php");
  exit();
}
...
if (isset($payload['mulai_pada']) && $payload['mulai_pada']) {
  $payload['mulai_pada'] = str_replace("T", " ", $payload['mulai_pada']);
}
if (isset($payload['selesai_pada']) && $payload['selesai_pada']) {
  $payload['selesai_pada'] = str_replace("T", " ", $payload['selesai_pada']);
}
...
if (table_has_column($koneksi, $table, 'slug')) {
  $nameSource = '';
  foreach (['judul', 'nama_event', 'nama', 'title'] as $key) {
...
if (isset($_FILES['gambar_sampul_url']) && is_uploaded_file($_FILES['gambar_sampul_url']['tmp_name'])) {
  ...
  $safeName = uniqid('evt_', true) . "." . strtolower($ext);
...
$kategoriOptions = fetch_options_by_table(
  $koneksi,
  ["kategori", "kategori_event", "kategori_destinasi"],
...
<div class="card">
  <div>
    ...
    <p class="page-sub">Tambah / edit event pariwisata Jogja</p>
  </div>
  ...
  <div class="toolbar">
    <div class="left">
      <button class="btn" onclick="openModal()">+ Tambah Event</button>
    </div>
    <form method="GET" style="display:flex;gap:10px;align-items:center">
      ...
      <a class="btn secondary" href="<?= BASE_URL ?>/admin/event.php">Reset</a>
    </form>
  </div>
    ...
    <table class="table2">
      ...
      <?php while($row = $data->fetch_assoc()): ?>
        <tr>
          ...
          <td>
            <small>
              <?= h($colMulai ? ($row[$colMulai] ?? '-') : '-') ?>
              <?php if ($colSelesai): ?> s/d <?= h($row[$colSelesai] ?? '-') ?><?php endif; ?>
            </small>
          </td>
          <td><span class="pill"><?= h($colStatus ? ($row[$colStatus] ?? '-') : '-') ?></span></td>
          <td><?= h(isset($row['harga']) ? number_format((float)$row['harga'], 0, ',', '.') : '-') ?></td>
          <td>
          ...
          <td class="actions">
            <a class="btn-sm gray" href="<?= BASE_URL ?>/admin/event.php?edit=<?= h($row[$pk]) ?>">Edit</a>
            <a class="btn-sm danger" onclick="return confirm('Hapus event ini?')" href="<?= BASE_URL ?>/admin/event.php?hapus=<?= h($row[$pk]) ?>">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
      </tbody>
...
<div class="modal" id="modal">
  <div class="modal-card">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <?php
        $mulaiVal = isset($edit['mulai_pada']) ? date('Y-m-d\TH:i', strtotime($edit['mulai_pada'])) : '';
        $selesaiVal = isset($edit['selesai_pada']) ? date('Y-m-d\TH:i', strtotime($edit['selesai_pada'])) : '';
        $statusVal = $edit['status'] ?? 'draft';
      ?>
      <div class="form-grid">
        <div>
        ...
        <div class="full">
          <label>judul *</label>
          <input type="text" name="judul" required value="<?= h($edit['judul'] ?? '') ?>">
        </div>
        <div>
          <label>lokasi</label>
          <input type="text" name="lokasi" value="<?= h($edit['lokasi'] ?? '') ?>">
        </div>
        <div>
        ...
        <div>
          <label>mulai_pada</label>
          <input type="datetime-local" name="mulai_pada" value="<?= h($mulaiVal) ?>">
        </div>
        <div>
          <label>selesai_pada</label>
          <input type="datetime-local" name="selesai_pada" value="<?= h($selesaiVal) ?>">
        </div>
        <div>
          <label>kuota</label>
          <input type="number" name="kuota" value="<?= h($edit['kuota'] ?? '') ?>">
        </div>
        <div>
          <label>harga</label>
          <input type="number" name="harga" value="<?= h($edit['harga'] ?? '') ?>">
        </div>
        <div class="full">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px;margin-top:10px;">
              <?php foreach ($detailImages as $img): ?>
                <div style="border:1px solid #eee;padding:6px;border-radius:10px;text-align:center;">
                  ...
                  <a class="btn-sm danger" style="margin-top:6px;display:inline-block;" onclick="return confirm('Hapus gambar ini?')" href="<?= BASE_URL ?>/admin/delete_detail_gambar.php?id_galeri=<?= h($img['id_galeri']) ?>&redirect=event&ref_id=<?= h($edit[$pk]) ?>">Hapus</a>
                </div>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>
