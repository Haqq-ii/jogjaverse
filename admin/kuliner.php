            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px;margin-top:10px;">
              <?php foreach ($detailImages as $img): ?>
                <div style="border:1px solid #eee;padding:6px;border-radius:10px;text-align:center;">
                  ...
                  <a class="btn-sm danger" style="margin-top:6px;display:inline-block;" onclick="return confirm('Hapus gambar ini?')" href="<?= BASE_URL ?>/admin/delete_detail_gambar.php?id_galeri=<?= h($img['id_galeri']) ?>&redirect=kuliner&ref_id=<?= h($edit[$pk]) ?>">Hapus</a>
                </div>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>
