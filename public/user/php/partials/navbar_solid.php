...
?>
<nav id="mainNavbar" class="navbar navbar-expand-lg fixed-top navbar-dark navbar-solid">
  <div class="container navbar-shell">
    <!-- Logo Brand -->
    <a class="navbar-brand fw-bold" href="<?= $base_url ?>/public/user/php/landingpageclean.php">
      Jogja<span class="brand-accent">Verse.</span>
    </a>
      ...
      <div class="navbar-account d-flex justify-content-center align-items-center gap-2">
        <?php if ($is_logged_in): ?>
          <a href="<?= $base_url ?>/public/user.php?tab=overview" class="d-flex align-items-center text-decoration-none">
            <?php if ($avatar_url !== ''): ?>
              <img src="<?= h(build_asset_url($avatar_url)) ?>" alt="Profile" class="nav-avatar">
            <?php else: ?>
              <span class="nav-avatar-initial">
                <?= h($avatar_initial) ?>
              </span>
            <?php endif; ?>
            <span class="text-white fw-medium d-none d-md-inline nav-account-label">Akun</span>
          </a>
        <?php else: ?>
